...
allbiases = tf.concat([tf.reshape(O1, [-1]), tf.reshape(O2, [-1]),
                       tf.reshape(O3, [-1]), tf.reshape(O4, [-1]), tf.reshape(B5, [-1])], 0)
# to use for sigmoid
allactivations = tf.concat([tf.reshape(
    Y1, [-1]), tf.reshape(Y2, [-1]), tf.reshape(Y3, [-1]), tf.reshape(Y4, [-1])], 0)
# to use for RELU
...
datavis = tensorflowvisu.MnistDataVis(
    title4="Logits", title5="activations", histogram4colornum=2, histogram5colornum=2)
...
md_path = R"C:\Users\yaoya\AppData\Local\conda\conda\envs\tensorflow\tensorflow-mnist-tutorial\TestProject\mdlib\md4\init"
